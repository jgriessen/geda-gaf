...
;;
;; Hacked to emit a stricter syntax supported by gnucap development
;; snapshot 20080210 by Peter Clifton
;;
;; --------------------------------------------------------------------------
...
(define verilog:write-module-declaration
  (lambda (module-name port-list p)
    (begin
      ...
      (display " (" p)
;      (newline p) ; gnucap doesn't like the \n whitespace
...
(define verilog:write-top-header
	(lambda (p)
	  (let ((port-list (verilog:get-port-list)))
	    (begin
	      (display "verilog" p) ; Throw gnucap into verilog mode
	      (newline p)
	      ...
	      (verilog:write-module-declaration verilog:get-module-name
						port-list p)
	      (newline p)
	      ; (verilog:write-port-directions port-list p) ; Not sure if gnucap likes or needs this
	      (newline p)))))
...
(define verilog:display-escaped-identifier
  (lambda (netname port)
	...
	(display netname port)))) ; just display the identifier,
			       ; gnucap doesn't "get" the escaping -
			       ; might not like the unescaped version either though!
		  ...
		  (begin
		    (let ((device (get-device package)))
 			  ...
 			  (begin
			    ...
			    (verilog:display-attributes package port)
 			    (display " " port)
...
;;
;; Remove any unwanted attributes from a list of those an entity has
;;
(define verilog:filter-wanted-attr
  (lambda (all-attr-names)

    (define ignore-attr-names (list "device" "refdes" "symversion"))

    (define is-ignored-attr-name?
      (lambda (name)
        (memv (string->symbol name)
              (map string->symbol ignore-attr-names)
      ))
    )

    (remove is-ignored-attr-name? all-attr-names)
  )
)


;;
;; output attributes for this package
;;
(define verilog:display-attributes
  (lambda (package port)
    (begin
      (let ( (attr-name-list (verilog:filter-wanted-attr (gnetlist:vams-get-package-attributes package)))
             (comma_pending #f) )
        (if (not (null? attr-name-list))
            (begin
              ;(newline port)
              (display " #(" port)
              (for-each
                (lambda (attr-name)
                  (begin
                    ...
                    (if comma_pending
                        (begin
                          (display "," port)
                          ; (newline port) ; gnucap doesn't like \n whitespace
                         )
                        (set! comma_pending #t)
                    )
                    (display "    ." port)
                    (display attr-name port)
                    (display "(" port)
                    (verilog:display-attribute package attr-name port)
                    (display ")" port)))
                attr-name-list
              )
            ; (newline port) ; gnucap doesn't like the \n whitespace
            (display "  )" port)))))))



;;
;; output a single named attribute for this package
;;
(define verilog:display-attribute
  (lambda (package attr-name port)
    (display (gnetlist:get-package-attribute package attr-name) port)
  )
)
...
;;;
(define verilog-gnucap
  (lambda (output-filename)
    (let ((port (open-output-file output-filename)))
      (begin
	(verilog:get-nets-once!)
	(verilog:write-top-header port)
	; (verilog:write-wires port) ; gnucap doesn't seem to need (or like) these
	; (verilog:write-continuous-assigns port) ; gnucap probably doesn't know these either
